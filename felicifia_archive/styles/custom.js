...
function sort(){
    var list = document.getElementsByClassName('topics')[0];
    var items = list.getElementsByClassName('row');
    var sortList = [];
    var key = whichChild(this);
    console.log(this);
    var c = this.classList;
    c.toggle('asc');
    var dir = c.contains('asc') ? 1 : -1;

    var x = this.parentElement.children;
    this.classList.add('active');
    x[(key+1)%3].classList.remove('active');
    x[(key+2)%3].classList.remove('active');
    ...
    sortList.sort(function (a, b) {
        var c = a.firstElementChild.children[key].getAttribute('data-sort'),
            d = b.firstElementChild.children[key].getAttribute('data-sort');

        return dir*(c < d ? -1 : c > d ? 1 : 0);
    });
...
while (!((c = p.classList).contains("post") || c.contains("panel"))) {
p = p.parentElement;
}
var x = parseInt(this.value.slice(4))-1;
var y=p.getElementsByClassName("postrev");

var archiveLink = p.getElementsByClassName("postlink")[0];
var inputs = document.querySelectorAll('input[type="hidden"]');
var idx = parseInt(y[x].getAttribute("data-snap"));
var t = inputs[idx].value.split("/")[0];
var link = inputs[idx].value.split("/").slice(1).join("e");

var text = document.createTextNode(" (Archived at " + t.slice(0,4) + "-" + t.slice(4,6) + "-" + t.slice(6,8) + "T" + t.slice(8,10) + ":" + t.slice(10,12) + ":" + t.slice(12,14) + ")");
archiveLink.replaceChild(text, archiveLink.childNodes[0]);
archiveLink.setAttribute("href", "https://web.archive.org/" + t + "/http://felicifia.org/" + link + "#" + p.id)

if (this.value[0] == "s"){
for (var j=0;j<y.length;j++)y[j].setAttribute("class","postrev hidden");
var d = p.getElementsByClassName("diff");
if (d.length) d[0].setAttribute("class","diff hidden");
y[x].setAttribute("class","postrev")
} else {
if (this.value[1] == "h") {
var base = y[x].innerHTML.replace(/(\n|<br ?\/?>)+/g, "\n\n");
var newtxt = y[x+1].innerHTML.replace(/(\n|<br ?\/?>)+/g, "\n\n");
} else {
y[x].setAttribute("class", "postrev shown");
y[x+1].setAttribute("class", "postrev shown");
var base = y[x].innerText;
var newtxt = y[x+1].innerText;
}
var diffoutputdiv = p.getElementsByClassName("diff")[0];
var d = dmp.diff_main(base,newtxt);
dmp.diff_cleanupEfficiency(d);
diffoutputdiv.innerHTML = dmp.diff_prettyHtml(d);
for (var j=0;j<y.length;j++)y[j].setAttribute("class","postrev hidden");
diffoutputdiv.setAttribute("class","diff");
}
}
document.addEventListener("DOMContentLoaded", function(){
    var p = [];
p = Array.prototype.concat.apply(p, document.getElementsByClassName("post"));
p = Array.prototype.concat.apply(p, document.getElementsByClassName("panel"));
    for (var i = 0; i < p.length; i++) {
        var h = p[i].getElementsByClassName("postrev");
        var archiveLink = document.createElement("a");
        archiveLink.setAttribute("class", "postlink");
        var inputs = document.querySelectorAll('input[type="hidden"]');
        var idx = parseInt(h[h.length - 1].getAttribute("data-snap"));
        var t = inputs[idx].value.split("/")[0];
        var link = inputs[idx].value.split("/").slice(1).join("e");

        var text = document.createTextNode(" (Archived at " + t.slice(0, 4) + "-" + t.slice(4, 6) + "-" + t.slice(6, 8) + "T" + t.slice(8, 10) + ":" + t.slice(10, 12) + ":" + t.slice(12, 14) + ")");
        archiveLink.appendChild(text);

        archiveLink.setAttribute("href", "https://web.archive.org/" + t + "/http://felicifia.org/" + link + "#" + p[i].id)
        p[i].getElementsByClassName("author")[0].appendChild(archiveLink);

        if (h.length < 2) continue;

        var selectList = document.createElement("select");
        p[i].getElementsByClassName("author")[0].appendChild(selectList);
        for (var j = 1; j < h.length; j++) {
            var option = document.createElement("option");
            option.value = "show" + j;
            option.text = "Show revision " + j;
            selectList.appendChild(option);
            if (p[i].getAttribute("class").indexOf("post")>=0) {
            option = document.createElement("option");
            option.value = "dtxt" + j;
            option.text = "\xa0\xa0Diff (text) " + j + " vs. " + (j + 1);
            selectList.appendChild(option);
            option = document.createElement("option");
            option.value = "dhtm" + j;
            option.text = "\xa0\xa0Diff (HTML) " + j + " vs. " + (j + 1);
            selectList.appendChild(option);
            }
        }
        var option = document.createElement("option");
        ...
        option.text = "Show revision " + j;
        selectList.appendChild(option);
        selectList.value = "show" + j;
        selectList.addEventListener("change", listener);
    }
    var x = document.getElementsByClassName("sort");
    for (var i = 0; i < x.length; i++) {
        x[i].addEventListener("click",sort);
    }
});
